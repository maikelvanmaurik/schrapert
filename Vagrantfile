...
Vagrant.configure("2") do |config|
  config.vm.define 'docs', autostart: true do |node|
    node.vm.box = 'ubuntu/trusty64'
    node.vm.hostname = "docs.schrapert.dev"
    node.vm.network :private_network, ip: '192.168.162.12'
    node.vm.network "forwarded_port", guest: 4000, host: 4000
   	node.vm.provision "shell", :path => 'provision/docs.sh', :privileged => false
   	node.vm.provision :shell, path: "provision/start-jekyll.sh", run: "always", privileged: false
