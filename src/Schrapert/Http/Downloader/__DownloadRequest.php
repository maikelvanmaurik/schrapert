 ...
 * @event end
 */
class DownloadRequest implements \Psr\Http\Message\RequestInterface, WritableStreamInterface
{
    ...
    public function handleEnd()
    {
        if($this->async) {
            ...
            list($response,) = $this->parseResponse($this->buffer);
            $this->emit('response', array($response, $this));
        }
    }
    ...
    public function getResponseFactory()
    {
        if (null === $factory = $this->responseFactory) {
            ...
            $factory = function ($request, $async, $stream, $protocol, $version, $code, $reasonPhrase, $headers, $body) {
                if(!$async) {
